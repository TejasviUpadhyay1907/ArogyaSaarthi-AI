...
|------|---------------------|-----------------|
| Languages | JavaScript, Java, SQL (MySQL) | Python, SQL basics |
| Frontend | HTML5, CSS3, Bootstrap 5 | — |
| Backend | Node.js, Express.js, REST APIs | REST APIs (Python), FastAPI |
| Database | SQLite (file-based, session logs), MySQL (schema design) | Reads/writes via API only |
| AI/ML | — | Pandas, NumPy, LLM basics, Prompt Engineering, RAG concepts, Multi-LLM Integration |
| DSA/CS | — | DSA, OOPs, DBMS, SE |
...
- **You (Python/AI owner):**
  - Python AI microservice (`app.py` using FastAPI, as in plan).
...
| API Gateway | Node.js + Express.js (REST APIs) | Teammate |
| AI Microservice | Python + FastAPI (`app.py`) | You |
...
| API Tools | Postman + Swagger (Express + FastAPI auto-docs) | Both |
| Deployment | Render.com (Node + Python) or local demo | Both |

> SQLite is used as-is for the prototype (zero setup, file-based). MySQL knowledge helps with schema design thinking but SQLite is the right call for a 5-day hackathon build.
...
│   └── app.py              # FastAPI app (called by Node backend)
│
└── README.md
```
...
- **Low-resource friendly:**
  ...
  - Python AI microservice runs a lightweight FastAPI app with rule-based logic.
...
**You (Python/AI):**
- [ ] Setup Python FastAPI app (`ai_engine/app.py`) with 3 endpoints:
...
- [ ] Wire these into FastAPI endpoints `/extract`, `/classify`, `/explain`.
...
**You (Python/AI):**
- [ ] Implement fallback mode in FastAPI:
...
- [ ] Add health-check route in FastAPI (for Node to verify connectivity).
...
- [ ] Deploy Python FastAPI on Render.com (free tier) or Railway.app.
- [ ] Host frontend on GitHub Pages or Netlify (static).
- [ ] If deployment is difficult, prepare local demo (Node + FastAPI running on a laptop).
...
### Technology Stack
HTML5, CSS3, Bootstrap 5, Vanilla JavaScript, Web Speech API, Node.js, Express.js, REST APIs, SQLite (file-based anonymized session logs only), Python, FastAPI, Optional OpenAI / Gemini API (explanation/extraction only), **deterministic rule-based triage (rules decide urgency and facility)**, Git/GitHub, Postman/Swagger, Render.com or similar (deployment)
...
| Feasibility | Uses well-known stacks (Node, FastAPI, SQLite) and can run on a single low-cost VM or laptop |
